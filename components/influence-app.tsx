...
function GoogleMark() {
  return <svg aria-hidden="true" viewBox="0 0 24 24" width="18" height="18"><path fill="#4285F4" d="M21.6 12.2c0-.7-.1-1.4-.2-2H12v3.9h5.4a4.7 4.7 0 0 1-2 3v2.6h3.3c1.9-1.8 2.9-4.4 2.9-7.5Z"/><path fill="#34A853" d="M12 22c2.7 0 5-.9 6.7-2.3l-3.3-2.6c-.9.6-2.1 1-3.4 1-2.6 0-4.8-1.8-5.6-4.1H3v2.7A10 10 0 0 0 12 22Z"/><path fill="#FBBC05" d="M6.4 14a6 6 0 0 1 0-3.9V7.4H3a10 10 0 0 0 0 9.3L6.4 14Z"/><path fill="#EA4335" d="M12 6c1.5 0 2.8.5 3.8 1.5l2.9-2.8A9.7 9.7 0 0 0 3 7.4l3.4 2.7C7.2 7.7 9.4 6 12 6Z"/></svg>;
}
...
function AuthScreen({ onAuthenticated }: { onAuthenticated: (user: UserSession) => void }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/google-status", { cache: "no-store" })
      ...
      .then((result: { enabled?: boolean }) => { if (active) setGoogleAvailable(Boolean(result.enabled)); })
  ...
  const authenticateWithGoogle = async () => {
    setError("");
    if (!googleAvailable) {
      setError("Google sign-in needs AUTH_GOOGLE_ID and AUTH_GOOGLE_SECRET in Vercel.");
      return;
    }
  ...
  return <main className="auth-shell">
    ...
    <section className="auth-panel">
      <div className="auth-card">
        ...
        <h2>Welcome back.</h2>
        <p>Sign in to continue to creator intelligence.</p>
        <button className="google-auth-button" type="button" onClick={authenticateWithGoogle} disabled={googleLoading || googleAvailable === null}><GoogleMark/>{googleLoading ? "Opening Google…" : "Continue with Google"}</button>
        <div className="auth-divider"><span>or use the demo account</span></div>
        <form onSubmit={authenticate} className="auth-form">
          ...
          <label><span>Password</span><input type="password" value={password} onChange={event => setPassword(event.target.value)} autoComplete="current-password" placeholder="Minimum 6 characters"/></label>
          {error && <div className="auth-error"><Icon name="shield" size={15}/>{error}</div>}
          <button className="primary-button wide auth-submit" disabled={loading}>{loading ? <><span className="button-spinner"/>Checking account…</> : <>Sign in to Influence<Icon name="arrow" size={16}/></>}</button>
        </form>
        ...
        <p className="auth-security"><Icon name="shield" size={13}/>Your profile name is read from the authenticated account—never hardcoded.</p>
      </div>
    </section>
  </main>;
}
